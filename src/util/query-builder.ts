...
export class QueryBuilder {
    ...
    //TODO: This is terrible code and I'm sorry for leaving this behind, but it was the quickest solution to get the job done.
    public eq_overload_parentheses(operand1: string, operand2: string, openingParenthesis: boolean, closingParenthesis: boolean): QueryBuilder {
        this.criteriaList.push(new SearchCriteria
        (openingParenthesis ? "( " + operand1 : operand1,
            closingParenthesis ? operand2 + " ) )" : operand2,
            CriteriaOperation.EQ, this.isCriteriaOr));
        this.isCriteriaOr = false;
        return this;
    }
    ...
    public criteria(cb: (builder: QueryBuilder) => QueryBuilder) {
        ...
        const built = cb(builder);
        if (built.criteriaList.length === 0) {
            return this;
        }
        this.criteriaList.push(built);
        return this;
    }
